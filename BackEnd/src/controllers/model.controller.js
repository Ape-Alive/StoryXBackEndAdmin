...
class ModelController {
  ...
  async createModelPrice(req, res, next) {
    try {
      ...
      const data = {
        ...
        // 确保数字字段是数字类型
        inputPrice:
          req.body.inputPrice !== undefined && req.body.inputPrice !== null && req.body.inputPrice !== ''
            ? parseFloat(req.body.inputPrice)
            : undefined,
        outputPrice:
          req.body.outputPrice !== undefined && req.body.outputPrice !== null && req.body.outputPrice !== ''
            ? parseFloat(req.body.outputPrice)
            : undefined,
        callPrice:
          req.body.callPrice !== undefined && req.body.callPrice !== null && req.body.callPrice !== ''
            ? parseFloat(req.body.callPrice)
            : undefined,
      }
   ...
   * 删除模型价格
   */
  async deleteModelPrice(req, res, next) {
    try {
      const { priceId } = req.params
      await modelService.deleteModelPrice(priceId, req.user?.id, req.ip)
      return ResponseHandler.success(res, { id: priceId }, 'Model price deleted successfully')
    } catch (error) {
      next(error)
    }
  }
