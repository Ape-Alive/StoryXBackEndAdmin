...
const getModelsValidator = [
  ...
  query('type').optional().isIn(Object.values(MODEL_TYPE)).withMessage('Invalid model type'),
...
const createModelValidator = [
  ...
  body('baseUrl').notEmpty().withMessage('Base URL is required').isString().withMessage('Base URL must be a string'),
...
const updateModelValidator = [
  ...
  body('type').optional().isIn(Object.values(MODEL_TYPE)).withMessage('Invalid model type'),
  body('baseUrl').optional().isString().withMessage('Base URL must be a string'),
  body('description').optional().isString().withMessage('Description must be a string'),
  body('isActive').optional().isBoolean().withMessage('isActive must be a boolean'),
  body('requiresKey').optional().isBoolean().withMessage('requiresKey must be a boolean'),
  body('apiConfig')
...
const getModelPricesValidator = [
  ...
  body('expiredEndDate').optional().isISO8601().withMessage('Invalid expired end date format'),
]
 ...
 * 删除模型价格验证
 */
const deleteModelPriceValidator = [
  param('id').notEmpty().withMessage('Model ID is required'),
  param('priceId').notEmpty().withMessage('Price ID is required'),
]
