...
class ApiKeyCreationService {
  ...
  async createGrsaiApiKey(config) {
    ...
    try {
      ...
      if (error.response) {
        const errorMsg = `Grsai API error: ${error.response.data?.msg || error.response.statusText}, status=${
  ...
  async createApiKeyForProvider(providerId, config) {
    ...
    logger.debug(
      `Creating API key for provider: id=${providerId}, name="${provider.name}", normalized="${providerName}"`
    )
    ...
    if (config.userId) {
      ...
      if (!apiKeyName.includes(config.userId)) {
        // 格式：原name_用户userId
        apiKeyName = apiKeyName ? `${apiKeyName}_用户${config.userId}` : `用户${config.userId}`
      }
    }
  ...
  async selectApiKeyForUser(userId, providerId) {
    ...
    logger.debug(
      `[ApiKeyCreation] Provider info: name=${provider?.name}, apiKeys=${provider?.apiKeys}, quota=${provider?.quota}`
    )
    ...
    logger.info(
      `[ApiKeyCreation] No API Key found from provider.apiKeys array, searching all system-level API Keys (userId=null)`
    )
